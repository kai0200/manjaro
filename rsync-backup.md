...
## rsync 命令
```
rsync -r source destination # 作为cp和mv命令的替代方法
rsync -r source1 source2 destination # source1、source2都会被同步到destination目录
rsync -a source destination # 可以递归同步以外，还可以同步元信息（比如修改时间、权限等）
rsync -anv source/ destination # 可以先用-n或--dry-run参数模拟执行的结果
rsync -av --delete source/ destination # delete
rsync -av --exclude='*.txt' source/ destination # 排除模式 排除.开头文件可以这样写--exclude=".*"
rsync -av --exclude={'file1.txt','dir1/*'} source/ destination # 多个排除模式也可以利用 Bash 的大扩号的扩展功能
rsync -av --exclude-from='exclude-file.txt' source/ destination # 写入一个文件，每个模式一行，然后用--exclude-from参数指定这个文件
```
